...
putlog "\[SB7\] Eggdrop: [lindex $::version 0] -- TCL: [info patchlevel] -- Library: [info library]"
array set sb7 [list]
...
#
# Eggdrop binds (example: 1.6.22): *E1* *E6* *E16* *E622* *E1622*
# TCL binds (example: 8.6.5): *T8* *T6*, *T86*, *T605*, *T8605*
...
# Note: there is only 1 core file now: CORE (sb7.tcl). The TAIL, DISP (dispatcher)
# & LOAD (loader) files are now part of CORE. The default settings file
# (sb6_permsettings_renameme.tcl) also has been deprecated.

# --- Boot it up! ---
set sb7(@fullfile) %SB7:FULLFILE% ; # Boolean (0/1)
if [string match %*% $sb7(@fullfile)] { set sb7(@fullfile) 0 }
# Will use this value later (in SB7:LOADBEADS)
...
##putlog "\[SB7:SETUP - CONFIG\] </setup>"
	...
	# Purge old beads first (autopurge)
	set autopurge [list sb7_tail.tcl]
...
#logme *
	...
	set dirs [file dirname [info nameofexecutable]]/scripts/sb7
	if [info exists ::sb7(@fullfile)] { if $::sb7(@fullfile) { empty dirs } }
	lappend dirs [file dirname [info nameofexecutable]]/scripts/sb7/[string tolower $::nick]
...
#putlog "\[EFFECTS\] A($a):TEXT($text)"
		switch -regexp -- [string tolower $a] {
			...
			{^hex\:to$} - hex:to { binary scan $text H* text ; set text [string toupper $text] }

			utf8 - {^utf\-8$} - {utf\-8} { set text [encoding convertto utf-8 $text] }
...
proc angle args {
	...
	set degree_mark \xC2\xB0 ; # UTF-8
	set degree_mark \xB0 ; # ASCII / ANSI
...
#[data array get @server chanmode:$mode]
				switch -exact -- $type {
					...
					default { error "\[SB7:RAW:MODE\] Unknown server data for channel mode: ${polarity}$mode (what type is it?)" }
